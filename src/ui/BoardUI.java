...
public final class BoardUI extends JFrame {
	...
	private final Image LOGO = new ImageIcon(getClass().getResource("/ui/images/logo.jpg")).getImage();//LOGO
	private static final Image rows = Toolkit.getDefaultToolkit().getImage("src/ui/images/rows.png");//行标识
	private static final Image cols = Toolkit.getDefaultToolkit().getImage("src/ui/images/cols.png");//列标识
	private static boolean vsRobots = true;//默认为人机对战
	private static Player human = Player.BLACK;//默认玩家执黑先行
	private List<ImageComponent> squares;//每个棋格都是一张图，所有棋格组成一个表
	private JPanel board;
	private JLabel whiteStat;
	private JLabel blackStat;
	private JLabel showTurn;
	private JRadioButtonMenuItem[] diffbuttons;//难度选择菜单项
	private JMenuItem newgame;//新游戏菜单项

	public BoardUI() {
		squares = new ArrayList<ImageComponent>(Board.BOARD_LENGTH * Board.BOARD_WIDTH);//棋格数量由棋盘大小决定
	...
	private void initComponents(Container pane) {
		...
		JRadioButtonMenuItem radiobutton;//组合选项菜单项
		...
		submenu = new JMenu("Opponent Inteligence");
		buttongroup = new ButtonGroup();
		radiobutton = new JRadioButtonMenuItem("BetaGo AI");
		radiobutton.addActionListener(new ActionListener() {
	...
	public void updateTurn(String player) {
		this.showTurn.setText(player+" Go");
	}

	public void declareDraw() {
		this.showTurn.setFont(showTurn.getFont().deriveFont(Font.BOLD));
		this.showTurn.setText("Draw !?");
		JOptionPane.showMessageDialog(this,"Hahaha，you are as smart as BetaGo!", "Draw!", JOptionPane.INFORMATION_MESSAGE);
	}

	public void declareWinner(String winnerName) {
		this.showTurn.setFont(showTurn.getFont().deriveFont(Font.BOLD));
		this.showTurn.setText(winnerName + "!");
		if(!winnerName.equals(human.toString())){
			JOptionPane.showMessageDialog(this,"Hahaha，BetaGo beats you up!", "You Lose!", JOptionPane.INFORMATION_MESSAGE);
		}
		else{
			JOptionPane.showMessageDialog(this,"Congratulations, you are smarter than BetaGo!", "You Win!", JOptionPane.INFORMATION_MESSAGE);
		}
	...
	public void notifyLostTurn(Player player) {
		JOptionPane.showMessageDialog(this, "No available moves for, " + player.toString()
						    + " turn lost", "Ups!", JOptionPane.INFORMATION_MESSAGE);
	}
